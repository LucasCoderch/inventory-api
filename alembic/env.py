...
def get_sync_url(async_url: str) -> str:
    # Alembic corre en modo sync.
    # Tu app usa asyncpg; para migraciones usaremos psycopg v3 (sync).
...
def run_migrations_offline() -> None:
    url = get_sync_url(settings.DATABASE_URL_ASYNC)
    context.configure(
        url=url,
...
def run_migrations_online() -> None:
    ...
    configuration["sqlalchemy.url"] = get_sync_url(settings.DATABASE_URL_ASYNC)
